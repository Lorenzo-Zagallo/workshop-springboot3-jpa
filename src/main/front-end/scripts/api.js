...
const state = {
    views: {
        list_users: document.querySelector("#usersList"),
...
async function main() {
    const viewElements = {
        'List of Users': loadUsers,
        'List of Products': loadProducts,
        'All the orders': loadOrders,
        'List of Categories': loadCategories
    };

    // verifica qual título corresponde e chama a função adequada
    const viewText = state.views.list_users.textContent || 
                     state.views.list_products.textContent || 
                     state.views.list_orders.textContent || 
                     state.views.list_categories.textContent;

    if (viewElements[viewText]) {
        await viewElements[viewText]();
    } else {
        console.log('Nenhuma ação encontrada para o estado atual.');
    }
}
...
document.getElementById('cadastroForm').addEventListener('submit', function(event) {
    ...
    const phone = document.getElementById('phoneRequest').value;
    const password = document.getElementById('passwordRequest').value;

    fetchUser(name, email, phone, password);
});

async function fetchUser(name, email, phone, password) {
    document.body.style.cursor = "wait";

    const userData = { name, email, phone, password };
    await apiService.postUsers(userData);
